...
//这几个宏只能在该类或者派生类中使用
#define INFO(...)       log_to_ui(QString("").sprintf(__VA_ARGS__), LOG_TYPE_INFO)
#define WARN(...)       log_to_ui(QString("").sprintf(__VA_ARGS__), LOG_TYPE_WARN)
#define ERROR(...)       log_to_ui(QString("").sprintf(__VA_ARGS__), LOG_TYPE_ERROR)
#define DEBUG(...)       log_to_ui(QString("").sprintf(__VA_ARGS__), LOG_TYPE_DEBUG)
...
public:
    ...
    void log_to_ui(QString s, int type = LOG_TYPE_INFO);
    void set_arg_by_uart(u_int8_t cmd);                       //根据串口数据设置参数
    void uart_cmd_reply_query(int type = 100);
...
signals:
    void s_log_to_ui(QString s, int type);                                  //输出信息到ui
