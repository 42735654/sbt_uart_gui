...
void sxdkl_uart_handler::init_self_widgets(int type, widgets_t *w)
{
    ...
    case WIFI_RESET:
        ...
        break;
    case BRING_NTC_ERROR:
        w->self_widget = new QPushButton(w->swi->widget_name);
        connect((QPushButton *)w->self_widget, &QPushButton::clicked, &hd, &sxdkl_uart_handler::bring_ntc_error);
        break;
    case CLEAN_NTC_ERROR:
        w->self_widget = new QPushButton(w->swi->widget_name);
        connect((QPushButton *)w->self_widget, &QPushButton::clicked, &hd, &sxdkl_uart_handler::clean_ntc_error);
        break;
    case BRING_WKQ_ERROR:
        w->self_widget = new QPushButton(w->swi->widget_name);
        connect((QPushButton *)w->self_widget, &QPushButton::clicked, &hd, &sxdkl_uart_handler::bring_wkq_error);
        break;
    case CLEAN_WKQ_ERROR:
        w->self_widget = new QPushButton(w->swi->widget_name);
        connect((QPushButton *)w->self_widget, &QPushButton::clicked, &hd, &sxdkl_uart_handler::clean_wkq_error);
        break;
        default:
            break;
    }
...
void sxdkl_uart_handler::bring_ntc_error()
{
    uart_stat_arg *ua = (uart_stat_arg *)arg;
    ua->ntc_error = 0xFE;
    uart_cmd_reply_query();
}
void sxdkl_uart_handler::clean_ntc_error()
{
    uart_stat_arg *ua = (uart_stat_arg *)arg;
    ua->ntc_error = 0;
    uart_cmd_reply_query();
}
void sxdkl_uart_handler::bring_wkq_error()
{
    uart_stat_arg *ua = (uart_stat_arg *)arg;
    ua->temp_error = 0xFE;
    uart_cmd_reply_query();
}
void sxdkl_uart_handler::clean_wkq_error()
{
    uart_stat_arg *ua = (uart_stat_arg *)arg;
    ua->temp_error = 0;
    uart_cmd_reply_query();
...
void sxdkl_uart_handler::update_arg(update_arg_type type)
{
    ...
    switch(type){
        case ARG_TO_UI:
            ...
            ua->light = ua->fb.light;
            ua->temp_check = ua->fb.temp_check;
            break;
    case UI_TO_ARG:
        ...
        ua->fb.light = ua->light;
        ua->fb.temp_check = ua->temp_check;
        break;
    default:
        break;
    }
